...
if ! command -v ufw >/dev/null 2>&1; then
  ...
  if [[ "$skip_ufw" =~ ^[Yy]$ ]]; then
    ...
    # DNS from LAN
    ufw allow from "$lan_subnet" to any port 53 comment "DNS from LAN"

    # HTTP from LAN
    ufw allow from "$lan_subnet" to any port 80 proto tcp comment "HTTP from LAN"
	
	# HTTPS from LAN
    ufw allow from "$lan_subnet" to any port 443 proto tcp comment "HTTPS from LAN"
	
	# DHCP
    ufw allow from "$lan_subnet" to any port 67 proto tcp comment "DHCP from LAN"
	
	# NTP
	ufw allow ntp comment "NTP Server"
	...
	if [[ -z "$vpn_interface" ]]; then
	  ...
	  # VPN port
	  ufw allow "$vpn_port"/"$vpn_proto" comment "VPN port"
	  
	  # SSH from VPN
      ufw allow from "$vpn_subnet" to any port 22 proto tcp comment "SSH from VPN"
	  
	  # DNS from VPN
      ufw allow from "$vpn_subnet" to any port 53 comment "DNS from VPN"
	  
	  # HTTP from VPN
      ufw allow from "$vpn_subnet" to any port 80 proto tcp comment "HTTP from VPN"
	  
	  # HTTPS from VPN
      ufw allow from "$vpn_subnet" to any port 443 proto tcp comment "HTTPS from VPN"
	...
	if [[ "$restrict_outgoing" =~ ^[Yy]$ ]]; then
	  ...
	  ufw allow out 67/tcp comment "Allow DHCP"
